...
const GAP_LIP_MM = 1;
/** Extra tooth reach on back jigs to accommodate slides shorter than nominal (1/4"). */
const BACK_TOOTH_EXTRA_MM = 0.25 * 25.4;
...
function computeSpineZones(
  ...
  for (let i = 0; i < sorted.length - 1; i++) {
    ...
    const zMin =
      sideAIsGap && !keepFullWidth ? -gapMm / 2 - GAP_LIP_MM : -clipTotalZ / 2;
    const zMax =
      sideBIsGap && !keepFullWidth ? gapMm / 2 + GAP_LIP_MM : clipTotalZ / 2;
...
export function buildJigSegmentGeometry(
  ...
  // Tooth depths: back jig has longer spacer teeth and short gap teeth.
  const spacerToothDepth = isBack
    ? backClearanceMm + BACK_TOOTH_EXTRA_MM + CLIP_LIP_DEPTH_MM
    : CLIP_LIP_DEPTH_MM;
  const gapToothDepth = isBack ? backClearanceMm : 0;
  // Total extent from edge for bottom cap sizing.
  const maxToothDepth = Math.max(spacerToothDepth, gapToothDepth);
  ...
  // Helper: build teeth for one side — long teeth at spacers, short at gaps.
  function addTeeth(zones: JigZone[], hasZones: boolean, wallZ: number) {
    if (hasZones) {
      for (const zone of zones) {
        const depth = zone.type === "spacer" ? spacerToothDepth : gapToothDepth;
        if (depth <= 0) continue;
        const yMm = zone.startY * INCHES_TO_MM;
        const hMm = zone.height * INCHES_TO_MM;
        geometries.push(
          box(depth, hMm, CLIP_WALL_MM, depth / 2, yMm + hMm / 2, wallZ),
        );
      }
    } else {
      // No zones = continuous wall (side panel outside face).
      // Only needs to be CLIP_LIP_DEPTH_MM deep to save material.
      geometries.push(
        box(
          CLIP_LIP_DEPTH_MM,
          segHeightMm,
  ...
  addTeeth(segment.sideAZones, sideAHasZones, innerWallZ);

  // ── 2. Outer wall (Z+ / sideB) ──
  addTeeth(segment.sideBZones, sideBHasZones, outerWallZ);
  ...
  // ── 6. Bottom cap (first segment only) ──
  // A plate that registers against the bottom edge of the panel.
  if (segment.segmentIndex === 0) {
    const capWidth = CLIP_WALL_MM + maxToothDepth;
    const capCenterX = (-CLIP_WALL_MM + maxToothDepth) / 2;
    geometries.push(
